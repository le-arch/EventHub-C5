...
const ticketTypeSchema = z.object({
  ...
  price: z.number({ message: 'Price must be a number' }).min(0, 'Price must be 0 or more'),
  quantityAvailable: z.number({ message: 'Quantity must be a number' }).min(1, 'At least 1 ticket required'),
})
...
export default function CreateEventPage() {
  ...
  const ticketForm = useForm<{ ticketTypes: TicketTypeForm[] }>({
    resolver: zodResolver(z.object({ ticketTypes: z.array(ticketTypeSchema).min(1, 'Add at least one ticket type') })),
    defaultValues: {
      ticketTypes: [{ name: '', price: 0, quantityAvailable: 1 }],
    },
  })
              ...
              <CardContent className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      <SelectTrigger className="bg-card border-purple-200 focus:border-purple-500 focus:ring-purple-500 "> 
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-3">
                  {fields.map((field, index) => {
                    const errorsForField = ticketForm.formState.errors.ticketTypes?.[index];
                    return (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              className="border-purple-200 focus:border-purple-500 focus:ring-purple-500"
                            />
                            {errorsForField?.quantityAvailable && (
                              <p className="text-xs text-red-500 mt-1"> {errorsForField.quantityAvailable.message}</p>
                            )}
                          </div>
                        </div>
                      </div>
                ...
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => append({ name: '', price: 0, quantityAvailable: 1 })}
